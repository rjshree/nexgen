...
const SCHEMA_GENERATOR_DIST = process.env.SCHEMA_GENERATOR_PATH || path.join(BASE_PATH, "schema-generator", "dist");
...
app.post("/generate-ui", async (req, res) => {
    const { projectName, muiColumns } = req.body;
    ...
    try {
        const result = await runGeneratorService(projectName, WORKSPACE, muiColumns);
...
app.post("/generate-api", async (req, res) => {
  const { projectName, apps, models, djangoModel, db }= req.body;
    ...
    try {
        const result = await runApiGeneratorService(projectName, WORKSPACE, djangoModel, db);
...
app.post("/generate-ui-skeleton", async (req, res) => {
    const { projectName, azureClientId } = req.body;
    console.log("Received API request to generate UI skeleton for project:", projectName);
    if (!projectName) {
        return res.status(400).json({ error: "projectName required" });
    }
    try {
        const result = await runUISkeletonGeneratorService(projectName, WORKSPACE, azureClientId);
...
app.post("/generate-api-skeleton", async (req, res) => {
    ...
    try {
       const result = await runAPISkeletonGeneratorService(projectName, WORKSPACE);
...
app.post("/start-dev-server",async (req, res) => {
    ...
    try {
      ...
      const npmCmd = isWindows ? "npm.cmd" : "npm";
       ...
       if (isWindows) {
        spawn("cmd", ["/c", "start", "cmd", "/k", "npm run dev"], {
          cwd: uiRoot,
          detached: true,
          stdio: "ignore",
        }).unref();
      ...
      if (isWindows) {
        spawn("cmd", ["/c", "start", "cmd", "/k", `${pythonExe} manage.py runserver 0.0.0.0:8000`], {
          cwd: apiRoot,
          detached: true,
          stdio: "ignore",
        }).unref();
      } else {
        spawn(pythonCmd, ["manage.py", "runserver", "0.0.0.0:8000"], {
          cwd: apiRoot,
          detached: true,
          stdio: "ignore",
        }).unref();
        ...
        console.log(`✅ Dev servers started — UI: ${uiPort}, API: ${apiPort}`);
        return res.json({ message: `Dev servers started, ${uiPort}, ${apiPort}` });
    } catch (err) {
